...
class Model {
  ...
  static async upsert(values, options) {
    ...
    if (options.hooks) {
      await this.hooks.run('afterUpsert', res, options);
    }
    return res;
  }
  ...
  static async bulkCreate(records, options = {}) {
    ...
    // Run before hook
    if (options.hooks) {
      await this.hooks.run('beforeBulkCreate', instances, options);
    }
    // Validate
    if (options.validate) {
    ...
    // Run after hook
    if (options.hooks) {
      await this.hooks.run('afterBulkCreate', instances, options);
    }
    return instances;
  }
  ...
  static async destroy(options) {
    ...
    if (options.individualHooks) {
      await Promise.all(instances.map(instance => this.hooks.run('afterDestroy', instance, options)));
    }
    // Run after hook
    if (options.hooks) {
  ...
  static async restore(options) {
    ...
    // Run afterDestroy hook on each record individually
    if (options.individualHooks) {
      await Promise.all(instances.map(instance => this.hooks.run('afterRestore', instance, options)));
    }
    // Run after hook
    if (options.hooks) {
  ...
  */
  ...
  async destroy(options) {
    ...
    if (this.constructor._timestampAttributes.deletedAt && options.force === false) {
      ...
      where[field] = Object.prototype.hasOwnProperty.call(attribute, 'defaultValue') ? attribute.defaultValue : null;
